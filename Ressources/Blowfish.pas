...
function EncryptECB(const Data: TBlowfishData; const Plain: TBytes): TBytes;

function DecryptECB(const Data: TBlowfishData; const Cipher: TBytes): TBytes;
...
var
  i: Integer;
begin
  Result := Copy(Plain, 0, Length(Plain));
  if Length(Result) mod 8 <> 0 then
    SetLength(Result, Length(Result) + (8 - (Length(Result) mod 8)));

  for i := 0 to (Length(Result) div 8) - 1 do
    BlowfishEncryptECB(Data, @Result[i * 8], @Result[i * 8]);
end;
...
var
  i: Integer;
begin
  if Length(Cipher) mod 8 <> 0 then
    raise Exception.Create('Invalid cipher length for ECB mode');

  SetLength(Result, Length(Cipher));

  for i := 0 to (Length(Cipher) div 8) - 1 do
    BlowfishDecryptECB(Data, @Cipher[i * 8], @Result[i * 8]);
end;
